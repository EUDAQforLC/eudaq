...
//         std::cout << "AHCALProducer::OnStopRun: _stopped==false loop" << std::endl;
//         eudaq::mSleep(100);
//         std::cout << ".";
//      }
      //wait for the all remaining data to be readout
...
// deque for events: add one event when new acqId is arrived: to be determined in reader
      ...
      while (true) {
         ...
         //      if Labview connection is not ready  just wait
         if (_fd <= 0 || _stopped) {
            myLock.unlock();
            ::usleep(1000);
            continue;
...
//            bufRead.clear();
//            SendEvent(RawDataEvent::EORE("CaliceObject", _runNo, _eventNo + 1));
